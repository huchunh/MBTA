...
create table Transit (
    transit_id number(10) constraint transit_id primary key,
    name varchar(30),
    status number,
    start_date date,
    price_per_ride float
)
/
create table Recharge_Device(
  recharge_device_id number(10) constraint recharge_device_id primary key,
  station_id number,
  status number,
  constraint fk_station foreign key (station_id) references station(station_id)
)
/
create table Transaction_Device(
  transaction_device_id constraint transaction_device_id primary key,
  station_id number,
  line_id number,
  status number,
  constraint fk_station foreign key (station_id) references station(station_id),
...
create table Transaction(
  transaction_id number(10) constraint transaction_id primary key,
  transaction_type varchar(10),
  swipe_time timestamp,
  wallet_id number not null,
  value number,
  transaction_device_id number,
  constraint fk_wallet foreign key (wallet_id) references wallet(wallet_id),
  constraint fk_transaction foreign key (transaction_device_id) references transaction_device(transaction_device_id)
)
/
