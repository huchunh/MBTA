...
CREATE TABLE STATION (station_id number(10)  primary key, name varchar (30), construction_date date);
CREATE TABLE LINE (line_id number GENERATED BY DEFAULT AS IDENTITY primary key, transit_id number REFERENCES Transit(transit_id), name varchar (30), start_date timestamp);
CREATE TABLE OPERATIONS (operation_id number GENERATED BY DEFAULT AS IDENTITY, primary key(operation_id), start_time timestamp, end_time timestamp, reason varchar(50), 
log_timestamp timestamp, transit_id number REFERENCES Transit(transit_id),line_id number REFERENCES Line(line_id),station_id number REFERENCES STATION(station_id), recharge_device_id number REFERENCES RECHARGE_DEVICE(Recharge_Device_id),transaction_device_id number REFERENCES Transaction_device(transaction_device_id)) ;
CREATE TABLE LINE_STATION_CONNECTIONS (connection_id number GENERATED BY DEFAULT AS IDENTITY, primary key(connection_id),  line_id number REFERENCES Line(Line_id), station_id number REFERENCES Station(Station_id));
