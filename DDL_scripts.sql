...
=======
             ...
             select 'Operations' table_name from dual union all
             select 'Line_station_connections' table_name from dual
>>>>>>> d3651e5b76f20b3b687e50a3f5e5c2b6e787691f
   )
   loop
...
<<<<<<< HEAD
CREATE TABLE LINE (line_id number GENERATED BY DEFAULT AS IDENTITY, FOREIGN KEY (transit_id) REFERENCES Transit(transit_id), name varchar (30), start_date datetime);
CREATE TABLE STATION (station_id number GENERATED BY DEFAULT AS IDENTITY, name varchar (30), contruction_date datetime);
CREATE TABLE OPERATIONS (operation_id number GENERATED BY DEFAULT AS IDENTITY, start_time datetime, end_time datetime, reason varchar(50), log_timestamp timestamp, FOREIGN KEY (transit_id) REFERENCES Transit(transit_id),FOREIGN KEY (line_id) REFERENCES Line(line_id), FOREIGN KEY (station_id) REFERENCES STATION(station_id), FOREIGN KEY (recharge_device_id) REFERENCES REACHARGE_DEVICE(Recharge_Device_id), FOREIGN KEY (transaction_device_id) REFERENCES Transaction_device(transaction_device_id) );
CREATE TABLE LINE_STATION_CONNECTIONS (connection_id number GENERATED BY DEFAULT AS IDENTITY, FOREIGN KEY (line_id) REFERENCES Line(Line_id), FOREIGN KEY (station_id) REFERENCES Station(Station_id) );
=======
